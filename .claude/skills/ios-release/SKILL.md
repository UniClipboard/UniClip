...
1. Bump the counter in `app.json` (the source of truth) with the script — do NOT
   hand-edit the numbers (that risks `ios.buildNumber` / `versionCode` drifting):
   ...
   prints the tag `v<version>.<build>`.
...
2. A local archive builds from the existing `ios/` prebuild output and does NOT
   ...
   ```bash
   APP_VARIANT=production npx expo prebuild -p ios --clean --no-install
   cd ios && pod install     # Podfile is regenerated by --clean
   ```
