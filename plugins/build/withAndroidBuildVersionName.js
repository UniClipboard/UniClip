...
const withAndroidBuildVersionName = (config) => {
    ...
    return (0, config_plugins_1.withAppBuildGradle)(config, (config) => {
        ...
        if (!versionNameRe.test(contents)) {
            console.warn('⚠ withAndroidBuildVersionName: no `versionName` line found in build.gradle; left unchanged');
            return config;
        }
        config.modResults.contents = contents.replace(versionNameRe, `versionName "${versionName}"`);
        console.log(`✓ Android versionName → "${versionName}" (iOS marketing version stays ${version})`);
        return config;
    });
};
exports.default = withAndroidBuildVersionName;
