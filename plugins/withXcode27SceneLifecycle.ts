...
export const patchAppDelegateForXcode27 = (contents: string): string => {
  ...
  if (
    windowStartupOccurrences !== 1 ||
    factoryCreationOccurrences !== 1 ||
    delegateAnchorOccurrences !== 1
  ) {
    throw new Error(
      `Unsupported Expo AppDelegate.swift: expected one factory creation, one window startup, and one ReactNativeDelegate anchor, found ${factoryCreationOccurrences}, ${windowStartupOccurrences}, and ${delegateAnchorOccurrences}`
    );
  }
