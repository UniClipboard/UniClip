...
function writeLog(
  level: keyof Pick<AppLogger, 'debug' | 'info' | 'warn' | 'error'>,
  source: string,
  args: unknown[]
): void {
  const prefix = `[${source}]`;
...
export function createLogger(source: string): AppLogger {
  ...
  if (!normalizedSource) {
    throw new Error('Logger source is required');
  }

  const existing = scopedLoggers.get(normalizedSource);
  if (existing) return existing;

  const scopedLogger: AppLogger = {
    debug: (...args) => writeLog('debug', normalizedSource, args),
    info: (...args) => writeLog('info', normalizedSource, args),
    warn: (...args) => writeLog('warn', normalizedSource, args),
    error: (...args) => writeLog('error', normalizedSource, args),
  };
  scopedLoggers.set(normalizedSource, scopedLogger);
  return scopedLogger;
}
