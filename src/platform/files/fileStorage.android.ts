...
import { calculateDirectorySize } from './fileStorage.shared';
...
export async function saveHistoryFile(
  ...
  try {
    ...
    // 检查文件是否已存在
    if (file.exists) {
      log.info('History file already exists:', file.uri);
      return file.uri;
    }
    ...
    log.info('History file saved:', file.uri);
    return file.uri;
  } catch (error) {
    log.error('Failed to save history file:', error);
    throw error;
  }
}
...
export async function getHistoryFileUri(
  ...
  try {
    ...
    log.error('Failed to get history file URI:', error);
    return null;
  }
}
...
export async function deleteHistoryFileDir(type: string, profileHash: string): Promise<void> {
  try {
    ...
    if (historyDir.exists) {
      historyDir.delete();
      log.info('History file directory deleted:', historyDir.uri);
    }
  } catch (error) {
...
export async function clearHistoryFiles(): Promise<void> {
  if (!HISTORY_BASE_DIR.exists) return;
  HISTORY_BASE_DIR.delete();
}

export async function getHistoryStorageSize(): Promise<number> {
  return calculateDirectorySize(HISTORY_BASE_DIR);
}

export async function cleanupOrphanedHistoryFiles(
  validProfileHashes: Set<string>
): Promise<number> {
  if (!HISTORY_BASE_DIR.exists) return 0;

  let cleanedCount = 0;
  for (const historyDir of HISTORY_BASE_DIR.list()) {
    if (!('isDirectory' in historyDir) || !historyDir.isDirectory) continue;

    const profileHash = (historyDir as Directory).name?.split('-').slice(1).join('-').toLowerCase();
    if (profileHash && !validProfileHashes.has(profileHash)) {
      (historyDir as Directory).delete();
      cleanedCount++;
    }
  }
  return cleanedCount;
...
export async function saveFile(
  ...
  try {
    ...
    // 检查文件是否已存在
    if (file.exists) {
      log.info('File already exists:', file.uri);
      return file.uri;
    }
...
export async function getFileUri(
  ...
  try {
    ...
    log.error('Failed to get file URI:', error);
    return null;
  }
}
...
export async function deleteFile(
  ...
  try {
    ...
    if (file.exists) {
      file.delete();
      log.info('File deleted:', file.uri);
    }
  } catch (error) {
...
export async function clearAllFiles(): Promise<void> {
  try {
    ...
    log.error('Failed to clear files:', error);
    throw error;
  }
}
...
export async function downloadAndSaveFile(
  ...
  try {
    ...
    // 直接下载到文件系统（不占用内存）
    log.info('Downloading file to:', file.uri);
    await File.downloadFileAsync(downloadUrl, file, {
      headers: headers || {},
    });

    log.info('File downloaded successfully:', file.uri);
    return file.uri;
  } catch (error) {
    log.error('Failed to download and save file:', error);
    throw error;
  }
}
