...
const log = createLogger('ApkDownload');
...
export function cleanOldApkCache(currentVersion: string): void {
  ...
  try {
    ...
    log.warn('cleanOldApkCache error:', e);
  }
}
...
export async function downloadApk(options: ApkDownloadOptions): Promise<string> {
  ...
  log.info(`Starting download source=${source} url=${url}`);

  // 确保缓存目录存在（含父目录）
  const cacheDir = getUpdateCacheDir(version);
  log.info(`Cache directory=${cacheDir.uri} exists=${cacheDir.exists}`);
  ensureUpdateDirExists(version);
  log.info('Cache directory ready');

  const destFile = getApkCachePath(version, asset.name);
  log.info(`Destination file=${destFile.uri} exists=${destFile.exists}`);
  // 删除已有的不完整文件
  if (destFile.exists) {
    destFile.delete();
    log.info('Deleted stale file');
  }

  const { nativeDownloadFile } = await import('android-util');
  log.info('Starting native file download');
  try {
  ...
  log.info('Download completed, file exists:', destFile.exists);

  // 校验哈希
  if (asset.sha256) {
    log.info(`Verifying hash, expected=${asset.sha256}`);
    const { nativeCalculateFileHash } = await import('android-util');
    ...
    log.info('No expected hash, skipping verification');
  }
