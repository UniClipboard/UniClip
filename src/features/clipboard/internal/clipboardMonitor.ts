...
export class ClipboardMonitor {
  ...
  private async loadPersistedHash(): Promise<void> {
    try {
      ...
      log.error('Failed to load persisted hash:', error);
    }
  }
  ...
  private async persistHash(content: ClipboardContent): Promise<void> {
    try {
      ...
      log.error('Failed to persist hash:', error);
    }
  }
  ...
  async start(): Promise<void> {
    ...
    log.info(`Started monitoring (${this.eventMonitorActive ? 'event-driven' : 'polling'})`);
  }
    ...
    try {
      ...
      if (changeCount !== null) {
        if (content) {
          ...
          if (hasContent) {
            log.info('Pasteboard read denied by user; pausing reads until changeCount changes');
      ...
      if (!content) {
        // log.info('Poll: clipboard is empty');
        return;
      }
  ...
  private async loadPersistedDeniedChangeCount(): Promise<void> {
    try {
      ...
      log.error('Failed to load denied changeCount:', error);
    }
  }
  ...
  private async setDeniedChangeCount(changeCount: number | null): Promise<void> {
    ...
    try {
      ...
      log.error('Failed to persist denied changeCount:', error);
    }
  }
  ...
  private async tryStartEventMonitor(): Promise<boolean> {
    ...
    try {
      ...
      log.warn('Event monitor unavailable, falling back to polling:', error);
  ...
  private async handleClipboardEvent(event: { type: string; content: string }): Promise<void> {
    ...
    try {
      ...
      log.error('Failed to handle clipboard event:', error);
    }
  }
  ...
  private handleAppStateChange = (nextAppState: AppStateStatus): void => {
    ...
    if (nextAppState === 'active') {
      ...
      if (!bgUploadEnabled) {
        // 应用进入后台，停止监听
        log.info('Background upload disabled, stopping polling (app went to background/inactive)');
        this.stopPolling();
      }
    }
  };
  ...
  async reset(): Promise<void> {
    ...
    try {
      ...
      log.error('Failed to clear persisted hash:', error);
    }
  }
}
...
const log = createLogger('ClipboardMonitor');
