...
import { activateRepository } from './activateRepository';
import { ensureHistoryItem } from '@/features/history';
import type { ClipboardContent } from '@/types/clipboard';
import { createLogger } from '@/support/observability';

const log = createLogger('ActivateClipboard');
...
export async function writeActivate(
  ...
  try {
    ...
    const row = await ensureHistoryItem(content);
    if (!row) return;

    await activateRepository.upsert(profileHash, row.contentId ?? null, Date.now());
  } catch (e) {
    log.error('writeActivate failed:', e);
  }
}
...
export async function clearActivate(): Promise<void> {
  try {
    ...
    log.error('clearActivate failed:', e);
  }
}
