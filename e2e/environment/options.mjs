...
export function parseOptions(args) {
  ...
  for (let i = 0; i < args.length; i += 2) {
    ...
    if (
      !["platform", "app", "scenario", "repeat", "peer-cli"].includes(key) ||
      !args[i + 1] ||
      args[i + 1].startsWith("--")
    )
      throw new Error(`Invalid option: ${args[i]}`);
    options[key === "peer-cli" ? "peerCli" : key] =
      key === "repeat" ? Number(args[i + 1]) : args[i + 1];
  }
  ...
  if (NETWORK_SCENARIOS.includes(options.scenario) && !options.peerCli)
    throw new Error(`${options.scenario} requires --peer-cli`);
  if (options.peerCli && !NETWORK_SCENARIOS.includes(options.scenario))
    throw new Error("--peer-cli requires a network scenario");
  if (options.scenario === "diagnostic-extensions" && options.platform !== "ios") throw new Error("diagnostic-extensions requires iOS");
  return options;
}
