...
class UcCoreModule : Module() {
    ...
    override fun definition() =
        ModuleDefinition {
            ...
            AsyncFunction("getHistoryPayload") {
                serverMap: Map<String, String>,
                profileId: String,
                trustInsecureCert: Boolean,
                ->
            ...
            AsyncFunction("healthProbe") {
                ...
                val report =
                    runBlocking {
                        getClient(trustInsecureCert)
                            .healthProbe(urls, trustInsecureCert, timeoutMs.toUInt(), networkEpoch.toULong())
                    }
                val results = mutableMapOf<String, String>()
                for ((url, result) in report.results) {
                    results[url] = healthProbeResultToString(result)
                }
                mapOf(
                    "networkEpoch" to report.networkEpoch,
                    "results" to results,
...
private fun clipboardKindFromString(kind: String?): ClipboardKind =
    when (kind) {
...
private fun syncSettingsFromMap(map: Map<String, Any?>): SyncSettings = SyncSettings(autoApply = map["autoApply"] as? Boolean ?: true)

private fun localContentFromMap(
    map: Map<String, Any?>,
    payload: ByteArray?,
): LocalContent =
    LocalContent(
...
private fun pullTriggerFromMap(map: Map<String, Any?>): PullTrigger =
    when (map["tag"] as? String) {
...
private fun upToDateReasonToString(r: UpToDateReason): String =
    when (r) {
...
private fun syncedMetaToMap(m: SyncedMeta): Map<String, Any?> =
    mapOf(
...
private fun stagedPreviewToMap(p: StagedPreview): Map<String, Any?> =
    mapOf(
...
private fun localContentToMap(c: LocalContent): Map<String, Any?> =
    mapOf(
...
private fun syncErrorToMessage(error: SyncException): String =
    when (error) {
        ...
        is SyncException.ProtocolException -> {
            "protocol error (HTTP ${error.status})"
        }

        is SyncException.DecodingFailed -> {
            "decoding failed: ${error.reason}"
        }
...
private fun syncOutcomeToMap(o: SyncOutcome): Map<String, Any?> =
    when (o) {
        is SyncOutcome.Uploaded -> {
            mapOf("tag" to "Uploaded", "meta" to syncedMetaToMap(o.meta))
        }
        ...
        is SyncOutcome.Staged -> {
            mapOf("tag" to "Staged", "preview" to stagedPreviewToMap(o.preview))
        }

        is SyncOutcome.UpToDate -> {
            mapOf("tag" to "UpToDate", "reason" to upToDateReasonToString(o.reason))
        }

        is SyncOutcome.BackingOff -> {
            mapOf("tag" to "BackingOff", "retryAfterMs" to o.retryAfterMs)
        }

        SyncOutcome.LoopDetected -> {
            mapOf("tag" to "LoopDetected")
        }

        is SyncOutcome.Failed -> {
            mapOf("tag" to "Failed", "error" to syncErrorToMessage(o.error))
        }
    }
...
private fun metaToMap(meta: ClipboardMeta): Map<String, Any?> =
    mapOf(
...
private fun historyRecordToMap(r: HistoryRecord): Map<String, Any?> =
    mapOf(
        "hash" to r.hash,
...
private fun clipboardKindToString(kind: ClipboardKind): String =
    when (kind) {
...
private fun probeResultToString(result: ProbeResult): String =
    when (result) {
...
private fun healthProbeResultToString(result: HealthProbeResult): String =
    when (result) {
        HealthProbeResult.SUCCESS -> "Success"
        HealthProbeResult.NOT_SUPPORTED -> "NotSupported"
        HealthProbeResult.UNREACHABLE -> "Unreachable"
    }

// Sync config type conversions (defaultSyncConfig / engineInit's SyncConfig arg)

private fun syncConfigToMap(c: SyncConfig): Map<String, Any> =
    mapOf(
...
private fun syncConfigFromMap(map: Map<String, Any?>): SyncConfig =
    SyncConfig(
