...
BUILD_SCRIPT="${UC_ENGINE_BUILD_TOOLS_ROOT:-$CORE_DIR}/bindings/uc-engine-uniffi/scripts/build-ios-xcframework.sh"
...
uc_engine_run_build "$BUILD_SCRIPT" "$CORE_DIR"
...
node "$ROOT_DIR/scripts/verify-unified-engine-core.mjs" \
  --record-local \
  --build-profile "$UC_ENGINE_UNIFFI_BUILD_PROFILE" \
